...
export default async function EditStockPage({params}: Props){
    ...
    // もしURLのIDが間違っているなどして、データが見つからなかった場合の分岐（エラーで画面が真っ白になるのを防ぎます）
    if(!stock) return <div>データが見つかりません。</div>


    // ----------------------------------------------------
    // 3. フォーム部品に渡すためのデータを準備（整形）
    // ----------------------------------------------------
    ...
    return(
        <div>
            ...
            <StockForm
                ...
                // 「今の状態」が詰まった初期データを渡します（これで最初から入力された状態になります）
