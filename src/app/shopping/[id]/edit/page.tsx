...
// 編集画面を作るメインの関数です。最新のルールに合わせて、URL（例: /shopping/5/edit）に入っている 5 という文字を、「Promise（あとで届く小包）」という形で受け取
export default async function EditShoppingPage({params}: {params: Promise<{id: string}>}) {
    // params）が届くのを await でしっかり待ち、中身を開けて resolvedParams という箱に入れ
    const resolvedParams = await params
    // URLから取り出したIDはただの「文字（String）」なので、データベースで探せるように「数字（Number）」に変換
    ...
    // フォームの「カテゴリ選択肢」を作るために、データベースからすべてのカテゴリをごっそり取得して categories に入れていま
    const categories = await prisma.category.findMany()

    // もし存在しないID（例: /shopping/9999/edit）を直接入力された場合、エラーで画面が真っ白にならないよう、「見つかりません」という文字だけを返して処理をストップ
    if(!item) {
    ...
    // editShoppingItem 関数に「このIDを更新してね！」という付箋をペタッと貼って、updateAction という新しい名前で準備
